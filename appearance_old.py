...
class AppearanceManager:
    ...
    def load_custom_themes(self, settings):
        ...
        if settings:
            try:
                ...
                custom_themes = settings.get("custom_themes", {})
                print(f"[DEBUG] Loading custom themes: {len(custom_themes)} themes found")
                for theme_id, theme_data in custom_themes.items():
                    print(f"[DEBUG] Theme {theme_id} secondary: {theme_data.get('secondary', 'NOT_FOUND')}")
    ...
    def get_theme_color(self, color_name, default=None):  # เพิ่ม default=None
        ...
        # เพิ่ม debug สำหรับ secondary color
        if color_name == "secondary":
            print(f"[DEBUG] get_theme_color secondary:")
            print(f"  Current theme: {self.current_theme}")
            print(f"  Theme data: {theme}")
            print(f"  Secondary in theme: {theme.get('secondary', 'NOT_FOUND')}")
            print(f"  Default provided: {default}")
        ...
        # เพิ่ม debug สำหรับ secondary color result
        if color_name == "secondary":
            print(f"  Final result: {color_value}")
