...
# เพิ่ม import สำหรับ PyQt5 และ swap_data
try:
...
_topmost_state = True  # ค่าเริ่มต้นเป็น True (ปักหมุด)
...
class NPCManagerCard:
    ...
    def __init__(
        ...
        self.style = {
            ...
            "success": "#34C759",
            "warning": "#FF9500",
            "info": "#3498DB",  # เพิ่มสีสำหรับปุ่ม Swap Data
        }
    ...
    def _update_section_description(self, section):
        ...
        if hasattr(self, "section_description_label") and hasattr(
            ...
            description = self.section_descriptions.get(section, "ไม่มีคำอธิบายสำหรับแท็บนี้")
    ...
    def _create_toolbar(self):
        ...
        # เพิ่ม hover effect สำหรับปุ่มสำรอง
        ...
        # เพิ่ม hover effect สำหรับปุ่ม Swap Data
    ...
    def _create_card_container(self):
        ...
        # สร้าง Scrollbar ก่อน (เหมือนเดิม)

        tree_scrollbar = ttk.Scrollbar(list_frame, orient="vertical")

        tree_scrollbar.pack(side="right", fill="y")

        # กำหนดสไตล์ให้ Treeview (ปรับสีให้เข้ากับธีม)
        ...
        style.configure(
            ...
            font=(self.font, self.font_size_small_bold),  # ปรับ font size ของ header
            padding=(15, 8),
    ...
    def _populate_treeview(self, items):
        ...
        # เพิ่มรายการแบบแบ่งชุด
        BATCH_SIZE = 50  # จำนวนรายการต่อชุด (ปรับตามความเหมาะสม)

        def add_batch(start_idx=0):
            # ตรวจสอบว่ายังมีรายการเหลืออยู่หรือไม่
            if start_idx >= len(items):
                # เสร็จสิ้นการเพิ่มรายการทั้งหมด
                self.window.update_idletasks()
                return
            ...
            # ตั้งเวลาเพิ่มชุดถัดไป - ใช้ window.after โดยตรงสำหรับ recursive function
    ...
    def _clear_detail_content_frame(self):
        ...
        try:
            # Force UI update และให้ window ได้รับ control กลับ
            self.window.update_idletasks()
            self.window.update()  # เพิ่ม update() เพื่อให้แน่ใจ
    ...
    def _on_search_change(self, *args):
        ...
        # ตั้งเวลาเล็กน้อยก่อนเริ่มค้นหาจริง (300ms)
    ...
    def show_window(self):
        ...
        # 🎯 เพิ่ม: หยุดการแปลก่อนแสดงหน้าต่าง NPC Manager เพื่อความปลอดภัย
        ...
        # ✅ ปรับปรุง: ให้แสดงบนสุดอย่างแน่นอน
        self.window.attributes("-topmost", True)  # บังคับ topmost ก่อน
    ...
    def _auto_add_new_character(self, character_name):
        ...
        try:
            ...
            # แสดงข้อความแจ้งเตือน
            self.flash_message(
                f"เพิ่มตัวละคร '{character_name}' ลงในฐานข้อมูลแล้ว!", "success"
            )
    ...
    def open_swap_data(self):
        """ปิดหน้าต่าง NPC Manager และเปิด UI สำหรับสลับฐานข้อมูล"""
        # ตรวจสอบ Dependencies
        if not HAS_PYQT5:
            self.flash_error_message("ไม่พบ PyQt5 กรุณาติดตั้ง: pip install PyQt5")
            return
        if not HAS_SWAP_DATA:
            self.flash_error_message("ไม่พบโมดูล Swap Data")
            return

        try:
            # FIX: 1. ปิดหน้าต่าง NPC Manager ตัวเองลงก่อน
            self.logging_manager.log_info("Closing NPC Manager to open Swap Data UI.")
            self.hide_window()

            # FIX: 2. จัดการ QApplication ให้ถูกต้องเพื่อป้องกันหน้าต่างนิ่ง
            if self.qt_app is None:
                # สร้าง instance ใหม่ถ้ายังไม่มี และเก็บไว้ใน self.qt_app
                self.qt_app = QApplication.instance() or QApplication(sys.argv)

            # 3. สร้างและแสดงหน้าต่าง Swap Data
            self.swap_window = SwapDataWindow(
                parent_callback=self.on_game_swapped,  # ส่ง callback ไปเหมือนเดิม
            ...
            self.flash_error_message("ไม่สามารถเปิด Swap Data ได้")

    def on_game_swapped(self):
        """Callback เมื่อสลับเกมสำเร็จ ทำหน้าที่แค่ส่งสัญญาณกลับไปให้ MBB ปิดโปรแกรม"""
        self.logging_manager.log_info(
