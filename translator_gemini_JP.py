...
class TranslatorGemini:
    ...
    def get_current_parameters(self):
        ...
        # สำหรับ Gemini JP จะแสดงชื่อรุ่นที่ง่ายต่อการอ่าน
        displayed_model = self.model_name + " (JP→TH)"
    ...
    def load_npc_data(self):
        try:
            ...
            print(f"TranslatorGemini JP: กำลังโหลดข้อมูล NPC จาก: {file_path}")
    ...
    def load_example_translations(self):
        # ตัวอย่างการแปลสำหรับภาษาญี่ปุ่น→ไทย 
        self.example_translations = {
            ...
            "すみません": "ขอโทษ",
            "お疲れ様": "เหนื่อยแล้วนะ",
            "そうですね": "ใช่เลย, นั่นสินะ",
            "わかりました": "เข้าใจแล้ว",
            "ちょっと": "นิดหน่อย, เดี๋ยวก่อน",
            "大丈夫": "ไม่เป็นไร",
            "頑張って": "สู้ๆ นะ",
            "お疲れ": "เหนื่อยแล้ว",
            "よろしく": "ฝากเนื้อฝากตัวด้วย",
            "待って": "เดี๋ยวก่อน!",
            "そうか": "อ๋อ งั้นหรือ",
        }
    ...
    def update_parameters(self, new_parameters):
        ...
        try:
            ...
            logging.info(f"TranslatorGemini JP: Updated parameters successfully: {new_parameters}")
    ...
    def is_translation_complete(self, original_text, translated_text):
        ...
        char_ratio = translated_char_length / original_char_length if original_char_length > 0 else 1
    ...
    def translate(
        self, text, source_lang="Japanese", target_lang="Thai", is_choice_option=False
    ):
        """
        ...
        try:
            ...
            prompt = (
                ...
                "Your task is to translate Japanese game text to Thai with these requirements:\n"
                "**IMPORTANT**: Even if the text appears mixed with English words or has unusual formatting, please attempt to translate any Japanese content you can identify.\n"
                "1. Translate the text COMPLETELY from Japanese, never cut off or omit any part of the original message\n"
                ...
                "5. **Use modern Thai vocabulary and expressions.** Always prioritize words that sound natural to modern Thai speakers.\n"
                "6. For very short text, treat it as either: phrase, exclamation, or name calling only\n"
                "7. Understand Japanese honorifics and politeness levels, but convert them to appropriate Thai equivalents\n"
                ...
                "11. **Japanese-specific considerations:**\n"
                "   - Convert Japanese keigo (敬語) appropriately - don't make it overly formal in Thai\n"
                "   - Handle particle nuances (は, が, を, に, etc.) naturally in Thai context\n"
                "   - Convert Japanese onomatopoeia to Thai equivalents when appropriate\n"
                "   - Understand Japanese sentence structure but make it flow naturally in Thai\n"
                "12. **IMPORTANT**: If you encounter mixed language text or unusual formatting, extract and translate any Japanese content you can identify\n"
                "13. VERY IMPORTANT: Return ONLY the Thai translation, DO NOT include the original Japanese text in your response\n"
                "14. **NEVER refuse to translate**: Always provide a translation attempt, even if the text seems unusual\n"
                f"Context: {context}\n"
                f"Character's style: {character_style}\n"
                f"{example_prompt}\n"
                "Special terms (use Thai explanations instead of translating):\n"
            )
    ...
    def translate_choice(self, text):
        ...
        try:
            ...
            logging.info(f"TranslatorGemini JP: translate_choice called with input text: '{text_preview}'")

            # ตรวจจับและตัดส่วนหัวข้อภาษาญี่ปุ่นออก
            text_to_translate = text.strip()
            choices_only = text_to_translate
            
            # รูปแบบที่พบบ่อยของคำถามในเกมญี่ปุ่น
            choice_headers = [
                "何と言いますか？",
                "何と言いますか?",
                "どう答えますか？",
            ...
            for header_pattern in choice_headers:
                if text_to_translate.startswith(header_pattern):
                    ...
                    header_found = True
                    logging.info(f"TranslatorGemini JP: Detected and removed header pattern: '{header_pattern}'")
                    break
                ...
                for header_pattern in choice_headers:
                    if header_pattern in text_to_translate:
                        ...
                        if len(parts) > 1:
                            ...
                            header_found = True
                            logging.info(f"TranslatorGemini JP: Found and removed header pattern in text: '{header_pattern}'")
                            break
            
            if not header_found:
                logging.info("TranslatorGemini JP: No Japanese choice header detected, translating full text")
            else:
                logging.info(f"TranslatorGemini JP: After header removal, choices text: '{choices_only[:100]}...'")

            # ถ้าไม่มีข้อความหลังตัด header ให้ใช้ข้อความเดิม
            if not choices_only.strip():
                choices_only = text_to_translate
                logging.info("TranslatorGemini JP: Empty choices after header removal, using original text")

            # ใช้ prompt สำหรับแปลเฉพาะตัวเลือกจากญี่ปุ่น
            prompt_parts = [
                "You are a highly skilled translator for the game Final Fantasy XIV.",
                "Translate the following player choice options from Japanese to Thai.",
                "Each choice option should be translated as a natural, concise Thai phrase suitable for game dialogue selection.",
                "CRITICAL REQUIREMENTS:",
                "1. Translate each choice option from Japanese into natural Thai.",
                "2. Preserve the newline structure exactly - each choice on its own line.",
                "3. Output ONLY the Thai translation. NO Japanese, NO explanations, NO prefixes.",
                ...
                "7. **Japanese considerations**: Handle keigo and politeness levels appropriately but keep Thai natural.",
                "8. Use modern, natural Thai expressions that sound appropriate for player responses.",
                "",
                f"CHOICE OPTIONS TO TRANSLATE (FROM JAPANESE):\n{choices_only}",
                "",
            ...
            completion_tokens_est = len(response.text) // 4 if hasattr(response, 'text') and response.text else 0
            ...
            if hasattr(response, "text") and response.text:
                ...
                # แยก replace operation ออกมาก่อน
                translated_choices = translated_choices.replace("Japanese:", "").replace("Japanese :", "")
                translated_choices = translated_choices.replace("จากญี่ปุ่น:", "").replace("ภาษาญี่ปุ่น:", "")

                # Basic cleaning
                translated_choices = translated_choices.replace("Thai:", "").replace("Thai :", "")
                translated_choices = translated_choices.replace("ไทย:", "").replace("ภาษาไทย:", "")
                ...
                for prefix in prefixes_to_remove:
                    if translated_choices.strip().startswith(prefix):
                        translated_choices = translated_choices.replace(prefix, "", 1).strip()
                        break
                ...
                logging.info(f"TranslatorGemini JP: Final choice translation result: '{final_preview}'")

                return final_result
            else:
                # แยก replace operation ออกมาก่อน
                text_error_preview = text[:70].replace('\n','<NL>')
                logging.error(f"TranslatorGemini JP: No response text from Gemini API for choice: '{text_error_preview}'")
            ...
            logging.error(f"TranslatorGemini JP: Error in translate_choice: {str(e)}, Input: '{text_exception_preview}'")
    ...
    def process_preset_area_c(self, ocr_text, preset_info=None):
        ...
        try:
            ...
            logging.warning(f"Lore processing failed: {e}, falling back to normal translation")
            return self.translate(ocr_text, source_lang="Japanese", target_lang="Thai")
    ...
    def force_retranslate(
        ...
        try:
            ...
            if hasattr(response, "text") and response.text:
                ...
                logging.warning("Force retranslation failed, returning previous translation")
